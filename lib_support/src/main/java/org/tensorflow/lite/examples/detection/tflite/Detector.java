...
public abstract class Detector {
    ...
    public List<Detection> recognizeImage(Bitmap bitmap, int sensorOrientation) {
        ...
        Log.d(TAG, "recognizeImage: Inizio riconoscimento immagine");

        //Load image
        Trace.beginSection("loadImage");
        Log.d(TAG, "recognizeImage: Inizio caricamento immagine");
        long startTimeForLoadImage = SystemClock.uptimeMillis();

        // TODO: check loadImage
        inputImageBuffer = loadImage(bitmap, sensorOrientation, 1f); // Load image (1 + 0 for augumentation)

        Log.d("DetectorActivity", "loadImage: Immagine processata");

        long endTimeForLoadImage = SystemClock.uptimeMillis();
        Log.d(TAG, "recognizeImage: Tempo di caricamento immagine: " + (endTimeForLoadImage - startTimeForLoadImage) + "ms");
        ...
        Log.d(TAG, "recognizeImage: Inizio inferenza");
        ...
        Trace.endSection();
        Log.d(TAG, "recognizeImage: Tempo di inferenza: " + (endTime - startTime) + "ms");
        ...
        // Ottieni la dimensione del primo asse (numero di box) e del secondo asse (numero di classi) dai tuoi array.
        int numDetections = outputBoxes[0].length;
        int numClasses = outputScores[0][0].length;
        Log.d(TAG, "recognizeImage: Numero di rilevamenti: " + numDetections + ", Numero di classi: " + numClasses);

        // Itera su tutte le rilevazioni (boxes) generate dal modello.
        for (int i = 0; i < numDetections; i++) {
            // Itera su tutte le classi (eccetto background) per il box corrente.
            for (int classIndex = 1; classIndex < numClasses; classIndex++) {
                ...
                if (score > 0.5f) {
                    ...
                    Log.v(TAG, "recognizeImage: Rilevamento - index: " + i + ", classIndex: " + classIndex + ", score: " + score);
                }

                // Filtra i risultati in base alla soglia per quella classe.
                if (score >= getDetectionThreshold(classIndex)) {
                    // Le coordinate del box: xmin, ymin, xmax, ymax (normalizzate).
                    float xminNormalized = outputBoxes[0][i][0];
                    float yminNormalized = outputBoxes[0][i][1];
                    float xmaxNormalized = outputBoxes[0][i][2];
                    float ymaxNormalized = outputBoxes[0][i][3];
                    ...
                    // Denormalizza le coordinate usando le dimensioni del bitmap.
                    ...
                    // Crea un oggetto RectF per la posizione.
                    RectF location = new RectF(xmin, ymin, xmax, ymax);

                    // Crea un nuovo oggetto Detection e aggiungilo alla lista.
                    detections.add(new Detection(
                            String.valueOf(classIndex), // ID della classe
                            getLabel(classIndex),        // Titolo della classe
                            score,                       // Confidence
                            location                     // Location
                    ));

                    Log.d(TAG, "recognizeImage: Aggiunto rilevamento - Class: " + getLabel(classIndex) + ", Score: " + score);
        ...
        Log.d(TAG, "recognizeImage: Totale rilevamenti dopo il filtraggio: " + detections.size());
    ...
    */
    ...
    private TensorImage loadImage(Bitmap bitmap, int sensorOrientation, float zoomRatio) {
        ...
        Log.d(TAG, "loadImage: Inizio caricamento immagine");
        Log.d(TAG, "loadImage: Dimensioni bitmap: " + bitmap.getWidth() + "x" + bitmap.getHeight());
        Log.d(TAG, "loadImage: Orientamento sensore: " + sensorOrientation + ", Zoom ratio: " + zoomRatio);
        ...
        Log.d(TAG, "loadImage: Bitmap caricato in inputImageBuffer");
        ...
        ImageProcessor imageProcessor =
                new ImageProcessor.Builder()
                        ...
                        //.add(new NormalizeOp(0.0f, 1.0f)) // Normalizza i valori dei pixel tra 0 e 1 come in Python

                        .build();

        Log.d(TAG, "loadImage: ImageProcessor costruito");
